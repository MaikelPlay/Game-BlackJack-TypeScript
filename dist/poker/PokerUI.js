...
export class PokerUI {
    constructor() {
        ...
        this.allinButton = document.getElementById('allin-button');
    ...
    updateDealerButton(dealerIndex, players) {
        if (players.length === 0)
            return;
    ...
    showTable(community, players, totalPot, phase) {
        if (this.communityCardsContainer) {
            ...
            if (currentCards !== community.length) {
                const communityHTML = community.map((c, idx) => `<div class="card dealt"><img src="${c.getImagen()}" alt="${c.toString()}"></div>`).join('');
            ...
            if (playerArea) {
                ...
                if (cardsDiv) {
                    ...
                    if (player.isHuman) {
                        ...
                        handHTML = player.holeCards.map(() => `<div class="card"><img src="assets/Baraja/atras.png" alt="Card back"></div>`).join('');
                    }
    ...
    markActivePlayer(playerIndex, players) {
    ...
    showPhaseOnActivePlayer(playerIndex, players, phase) {
    ...
    showBlindIndicators(smallBlindIndex, bigBlindIndex, players) {
    ...
    async promptPlayerAction(player, minCall, canRaise, lastBet, minRaise) {
    ...
    enableActionButtons(minCall, playerStack, canRaise, lastBet, minRaise) {
        if (this.foldButton)
        ...
        if (minCall === 0) {
            ...
            if (this.callButton)
                this.callButton.disabled = (playerStack === 0);
        }
    ...
    disableActionButtons() {
        ...
        if (this.callButton)
            this.callButton.textContent = this.translations.callButton;
    }
    ...
    setLanguage(lang) {
        ...
        if (this.potDiv)
            this.potDiv.textContent = `${this.translations.potLabel}:`;
        if (this.foldButton)
            this.foldButton.textContent = this.translations.foldButton;
        if (this.checkButton)
            this.checkButton.textContent = this.translations.checkButton;
        if (this.callButton)
            this.callButton.textContent = this.translations.callButton;
        if (this.raiseButton)
            this.raiseButton.textContent = this.translations.raiseButton;
    }
    ...
    showWinnerModal(winnerName, handDescription, amount) {
        ...
        if (modal && winnerNameEl && winnerHandEl && winnerAmountEl && continueBtn) {
            ...
            // Configurar el botón de continuar - solo cierra el modal
            newContinueBtn.addEventListener('click', () => {
                modal.classList.add('hidden');
                this.soundEffects.playClick();
            });
            // Auto-cerrar después de 4 segundos si el usuario no hace clic
            setTimeout(() => {
    ...
    limpiarTablero() {
        ...
        const allPlayerCardsDivs = document.querySelectorAll('.player-cards');
        allPlayerCardsDivs.forEach(cardsDiv => {
            cardsDiv.innerHTML = '';
        });
        const allPlayerStatusDivs = document.querySelectorAll('.player-status');
