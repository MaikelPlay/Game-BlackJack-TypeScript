...
export class BlackjackGame {
    constructor(ui, numeroJugadores, carteraInicial, nombreHumano = '', lang = 'es') {
        this.ui = ui;
        ...
        this.lang = lang;
        if (typeof this.ui.setLanguage === 'function') {
    ...
    nuevaRonda() {
        ...
        if (typeof this.ui.actualizarTipos === 'function') {
            this.ui.actualizarTipos(this.jugadores.map((j) => j.esHumano ? 'Humano' : 'IA (conservadora)'));
        }
        if (typeof this.ui.actualizarNombres === 'function') {
    ...
    realizarApuesta() {
        ...
        if (!humanCanBet) {
            ...
            this.jugadores.forEach(j => { if (!j.esHumano && j.apuestaActual > 0)
                j.ganar(j.apuestaActual); j.apuestaActual = 0; });
            this.ui.actualizarCarteras(this.jugadores.map(j => j.cartera));
            this.ui.actualizarApuestas(this.jugadores.map(j => j.apuestaActual));
            return;
        }
    ...
    actualizarVista(ocultarCartaCrupier = true) {
        ...
        try {
            ...
            this.ui.marcarTurno(this.estado === 'JUGANDO' ? this.jugadorActualIndex : null);
        }
        catch (e) {
            // noop
        }
    ...
    pedirCarta() {
        ...
        if (!jugadorActual.esHumano)
            return; // Solo el humano puede solicitar a través de la UI
        const card = this.baraja.robar();
        if (card)
    ...
    plantarse() {
        ...
        const jugadorActual = this.jugadores[this.jugadorActualIndex];
        if (!jugadorActual.esHumano)
            return; // Solo el humano puede plantarse a través de la UI
    ...
    determinarGanadores() {
        ...
        this.jugadores.forEach((jugador, i) => {
            ...
            if (puntJugador > 21) {
                ...
                if (typeof this.ui.mostrarResultadoJugador === 'function') {
                    this.ui.mostrarResultadoJugador(i, 'lose', apuestaJugador);
                }
            }
                ...
                if (typeof this.ui.mostrarResultadoJugador === 'function') {
                    this.ui.mostrarResultadoJugador(i, 'win', ganancia);
                }
            }
                ...
                if (typeof this.ui.mostrarResultadoJugador === 'function') {
                    this.ui.mostrarResultadoJugador(i, 'push', 0);
                }
            }
            else {
