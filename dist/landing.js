class LandingPage {
    constructor() {
        ...
        this.solitaireOption = document.getElementById('solitaire-option');
        this.memoryOption = document.getElementById('memory-option');
    ...
    configurarSelectorIdioma() {
        ...
        const applyColorAndTranslations = (valParam) => {
            ...
            this.languageSelect.style.color = color;
            this.languageSelect.style.borderColor = color;
            this.applyTranslations();
        };
    ...
    applyTranslations() {
        ...
        const balanceLabel = document.querySelector('label[for="balance-input"]');
        const nameLabel = document.querySelector('label[for="player-name-input"]');
        const playersLabel = document.querySelector('label[for="player-count-input"]');
        if (balanceLabel)
        ...
        if (this.startGameButton)
            this.startGameButton.textContent = t.start;
        if (this.blackjackOption)
            this.blackjackOption.textContent = t.blackjack;
        if (this.pokerOption)
            this.pokerOption.textContent = t.poker;
        if (this.solitaireOption)
            this.solitaireOption.textContent = t.solitaire;
        if (this.memoryOption)
            this.memoryOption.textContent = 'Memory';
    }
    ...
    seleccionarJuego(juego) {
        ...
        if (juego === 'Solitaire' || juego === 'Memory') {
            ...
            if (balanceSelection)
                balanceSelection.style.display = 'block';
            if (playerSelection)
